...
func registerAuthRoutes(api *gin.RouterGroup, adaptors *adaptor.Adaptor, cfg *utils.Config) {
	...
	auth := api.Group("/auth")
	{
		...
		auth.POST("/login", middleware.LoginRateLimit(cfg.App.Env), adaptors.AuthAdaptor.Login)
		auth.POST("/login-email", middleware.LoginRateLimit(cfg.App.Env), adaptors.AuthAdaptor.LoginWithEmail)
